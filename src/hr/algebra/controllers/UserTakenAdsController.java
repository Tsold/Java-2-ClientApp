...
public class UserTakenAdsController implements Initializable {
    ...
    private void initObservables() throws Exception {
        ...
        AdvertisementRepo.getInstance().setAds(ads, mainUser.getIdUser());
    }

    private void initTableCells() {

        tvAds.setItems(AdvertisementRepo.getInstance().getTakenAds(mainUser.getIdUser()));
    ...
    private void reserveAd(MouseEvent event) throws Exception {
        ...
        if (selectedIndex != -1) {
            ...
            alert.setHeaderText("Cancel session?");

            alert.setContentText(vehicle.toFullInfo());
            if (alert.showAndWait().get() == ButtonType.OK){
                AdvertisementRepo.getInstance().getAds().remove(remAd);
                AdvertisementRepo.getInstance().getTakenAds(mainUser.getIdUser()).remove(selectedIndex);
                repo.updateNotTakenAd(Id);
            }
        }
    ...
    private void deserialize() {
        ...
        if (file != null) {
            try {
                ...
                Logger.getLogger(UserTakenAdsController.class.getName()).log(Level.SEVERE, null, ex);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void setUserFromLogin(User user) throws Exception {
        mainUser = user;
